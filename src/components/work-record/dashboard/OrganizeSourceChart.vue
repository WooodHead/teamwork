...
export default {
  ...
  data () {
    return {
      ...
      initOption: {
        title: {
          left: 'left',
          text: this.$t('workRecord.dashboard.organizeSourceSumHour', { orgName: this.$t('workRecord.dashboard.group') })
        },
        tooltip: {
          trigger: 'item',
          triggerOn: 'mousemove',
  ...
  computed: {
    ...
    option () {
      let that = this, initData = {}, data = [], dicValues = [], allKeys = []
      ...
      if (that.querys.organize) {
        ...
        that.initOption.title.text = that.$t('workRecord.dashboard.organizeSourceSumHour', { orgName: '' })
      } else {
        chart = chart.filter(r => r.objectId === 1)
      }
      if (chart.length) {
        that.showChart = true
        initData = chart.map(r => { return r.status.SourceHours })
      }
      if (initData.length) {
        allKeys = _.without(_.union(_.flattenDeep(initData.map(r => { return _.keys(r) }))), '')
        _.forEach(allKeys, r => {
          dicValues = _.flattenDeep(initData.map(n => { return n[r] && n[r].Hours }))
          data.push({
            'name': r,
            'value': Math.abs(_.sumBy(dicValues).toFixed(2))
          })
        })
      }
