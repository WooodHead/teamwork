...
export default {
  ...
  data () {
    return {
      ...
      initOption: {
        title: {
          left: 'left',
          text: this.$t('workRecord.dashboard.organizeDQRank')
        },
        tooltip: {
          trigger: 'axis',
  ...
  computed: {
    ...
    ...mapGetters('taskchart', ['taskchartFilterBySourceOrWorkType', 'taskchartFilterByDate', 'taskchartFilterByOrganize']),
    option () {
      let that = this
      if (that.organizeTaskCharts && that.organizeTaskCharts.length > 0) that.showChart = true
      let initChart = that.taskchartFilterByDate(that.organizeTaskCharts, that.querys.date), chart = []
      // 机构过滤
      if (that.querys.organize) {
        that.initOption.title.text = that.$t('workRecord.dashboard.organizeRank')
        initChart = that.taskchartFilterByOrganize(initChart, that.querys.organize)
      } else if (that.querys.reset) {
        initChart = that.taskchartFilterByOrganize(initChart, that.initOrgIds)
      } else {
        initChart = that.taskchartFilterByOrganize(initChart, that.selectOrgIds)
      }
      chart = _.cloneDeep(initChart)
      // 来源或工作类型或人均数过滤
      if ((that.querys.source && that.querys.source.length) || (that.querys.workType && that.querys.workType.length) ||
        ...
        if ((that.querys.source && that.querys.source.length) || (that.querys.workType && that.querys.workType.length)) {
          chart = that.taskchartFilterBySourceOrWorkType(chart, that.querys)
  ...
  methods: {
    ...
    init () {
      ...
      this.loadWorkRecordOrganizeTaskCharts({ query: query, parentId: this.queryOrgId }).then(res => {
        if (this.isFirstLoad) this.initOrgIds = res.map(r => { return r.objectId })// 重置时需要initOrgIds
        this.selectOrgIds = res.map(r => { return r.objectId })
