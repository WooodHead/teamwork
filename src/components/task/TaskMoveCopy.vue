<template>
  <resource-move-copy
    ...
    :category="category"
    :objectID="+objectID"
    :disableDefaultBtns="task && task.type !== 'list'"
    :cardStyle="task && task.type === 'item'?'torn':'whole'"
    @selectItem="onSelectItem"
  >
    <!-- 子项的选择卡片 -->
    <q-card-section
      v-if="task&&task.type !== 'list'"
      slot="subSelect"
...
export default {
  ...
  computed: {
    ...
    taskLists () {
      return this.actionType === 'move' ? this.allTaskList.filter(task => task.id !== this.task.parentId) : this.allTaskList
    },
    hintLabel () {
  ...
  methods: {
    ...mapActions('task', ['loadTask', 'loadTaskLists', 'moveTask', 'copyTask']),
    action (targetId, objectID, objectType) {
      ...
      this.$store.dispatch(`task/${this.actionType}Task`, query).then(
        this.$router.push({
          name: 'taskDetail',
          params: {
            category: objectType,
    ...
    // 切换资源：其他项目/其他团队下的项目
    onSelectItem (val) {
      this.isNotSpecific = val.isNotSpecific// 选择资源（产品，团队，订单，项目）为true,选择具体项目为false
