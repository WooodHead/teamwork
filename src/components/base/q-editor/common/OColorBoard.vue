<template>
  <section class="o-color-board">
    <div
      class="row color-row"
      v-for="(row, i) in colorSet"
      :key="`row-${i}`"
    >
      <div
        class="row color-column"
        v-for="(col, j) in row"
        :key="`col-${j}`"
      >
        <div
          class="row justify-center items-center cell-container"
          @click="onSelect(col)"
        >
          <div
            class="row justify-center items-center cell"
...
<script>
import { highlightColor } from '@/components/base/q-editor/utils/shared'
