<template>
  <o-block-card
    ...
    @toggle-screen="onToggleFullScreen"
  >
    <template slot="toolbar-left">
      <div class="row col items-center full-height q-px-sm providers">
        <q-icon name="mdi-sitemap" /> {{$t('diagram.name')}}
      </div>
    </template>
    ...
    <template slot="toolbar-right">
      ...
      >
        <q-list>
          <q-item
            v-for="(item, index) of mermaidDiagrams"
            :key="index"
            @click.native="onSelectTemplate(item)"
            clickable
            v-close-popup
          >
            <q-item-section
    ...
    <section class="row col-12 diagram">
      <section
        class="col source"
        v-if="(mode==='edit' || fullScreen) && view.editable"
      >
        <codemirror
        ...
        <div
          ...
          v-if="error"
        >
          <q-icon name="info" /> 语法错误
        </div>
      </section>
      <section
        class="col preview"
        v-if="mode==='preview' || fullScreen"
      >
        <q-scroll-area
          class="scroll-area"
          v-if="fullScreen"
...
<script>
import OBlockCard from '@/components/base/q-editor/common/OBlockCard'
...
import { MermaidTemplates } from '@/components/base/q-editor/data/diagrams'
...
export default {
    ...
    toggleMode () {
      try {
        ...
        if (this.mode === 'preview') {
          this.renderMermaid()
          this.$parent.editor.options.onUpdate({ getJSON: '', getHTML: '', state: '', transaction: '' })
        } else {
          setTimeout(() => {
  ...
  computed: {
    ...
    toggleLabel () {
      return this.mode === 'edit' ? this.$t('label.preview') : this.$t('label.edit')
    },
    contentClass () {
    ...
    mermaidDiagrams () {
      return [
        { label: this.$t('diagram.flow'), value: 'flow' },
        { label: this.$t('diagram.sequence'), value: 'sequence' },
        { label: this.$t('diagram.class'), value: 'class' },
        { label: this.$t('diagram.state'), value: 'state' },
        { label: this.$t('diagram.gantt'), value: 'gantt' }
      ]
    }
  },
  mounted () {
...
<style lang="scss">
  ...
  .editor__content .ProseMirror .o-diagram {
    padding-top: 2.5rem;
  }
  ...
  .ProseMirror .o-diagram .o-card-content {
