<template>
  ...
  >
        ...
        <template v-slot:prepend>
          <div class="row items-center">
            <svg
              class="icon"
              aria-hidden="true"
              v-if="service.svgIcon"
            >
              <use :xlink:href="`#icon-${service.svgIcon}`"></use>
            </svg>
            <q-icon
              :name="service.icon || 'apps'"
              :color="service.color"
              v-else
            />
            <q-tooltip
              anchor="top middle"
              self="bottom middle"
            >{{$t('embed[service.value]')}}</q-tooltip>
          </div>
        </template>
      </q-input>
    </template>
    <template slot="toolbar-right">
      ...
      >
        <span
          class="text-blue"
          slot="label"
        >{{$q.lang.label.ok}}</span>
        <q-list style="min-width: 120px;">
          <o-common-item
            icon="settings"
            :label="$t('label.settings')"
            v-if="service.settings"
          >
            <q-menu
              ref="imgRef"
              anchor="top right"
              self="top left"
              content-class="o-menu o-embed-settings-menu"
            >
              <section
              ...
              >
                <div class="row justify-between items-center text-bold q-py-sm">
                  <div>
                    {{service.label}} {{$t('label.settings')}}
                  </div>
                  <div>
                    <q-btn
                      icon="close"
    ...
    >
      ...
      <div
        class="row justify-center items-center iframe"
        v-show="loading"
      >
        <q-spinner
          color="primary"
...
import { getEmbedService, getServiceSrc, EmbedServiceLink } from '@/components/base/q-editor/data/embed'
import { updateQueryStringItem } from '@/components/base/q-editor/utils/uri'
    ...
    onConfirm () {
      ...
      if (!result.validLink || !result.validId) {
        this.$q.notify({
          ...
          message: `${this.$t(embed[this.service.value])}, ${this.$t('embed.linkWarning')}`,
          position: 'top',
          timeout: 1500
        })
        return
