<template>
  ...
  >
    ...
    <span
      :style="`width:${node.attrs.width};height:${node.attrs.height}`"
      class="image-container fit q-my-xs"
    ...
    >
      ...
      >
        <template v-if="file">
          <img
            ...
            @click="selectImage"
            :alt="file?file.title:'这是一张图片'"
          />
        </template>
      </viewer>
      <!-- 附件内容展示 -->
      <div
        v-else-if="node.attrs.file===null&&node.attrs.src===''"
        v-html="attachHtml"
      />
      <!-- 其他格式 -->
      <attach-card
      ...
      <!-- 四个角角 -->
      <div
        v-if="view.editable&&isImage&&(selected || resizing)"
        class="image-resizer"
      >
        <span
    ...
    <!-- 图片设置和全屏 -->
    <template v-if="isImage&&node.attrs.caption">
      <div class="text-center text-grey caption">
        {{node.attrs.caption}}
      </div>
        ...
        <div class="col-auto actions">
          ...
          >
            <q-tooltip
              anchor="top middle"
              self="bottom middle"
            >{{$q.lang.editor.toggleFullscreen}}</q-tooltip>
          </q-btn>
          <q-btn
            color="blue"
            icon="settings"
                ...
                <div class="row col-12 items-start q-py-md">
                  ...
                  <div class="q-px-sm">
                    <q-btn
                      ...
                      flat
                    >
                      <q-tooltip>{{keepRatio ? $t('image.lockAspectRatio') : $t('image.unlockAspectRatio')}}</q-tooltip>
                    </q-btn>
                  </div>
                  <q-input
...
// import { resolveImg } from '@/components/base/q-editor/utils/image'
import { clamp } from '@/components/base/q-editor/utils/shared'
...
export default {
  ...
  computed: {
    ...mapState('file', ['imgExts', 'pdfExts', 'officePreviewExts', 'threeDExts', 'videoExts', 'audioExts']),
    src: {
      get () {
    ...
    attachHtml () {
      let classLength = this.node.attrs.dom.getElementsByClassName('image-container').length
      return classLength
        ? this.node.attrs.dom.getElementsByClassName('image-container')[classLength - 1].firstChild.innerHTML
        : node.attrs.dom.innerHTML
    },
    file () {
    ...
    isImage () {
      if (this.file) {
        return this.imgExts.includes(this.file.extension.toLowerCase())
      } else {
        return this.node.attrs.src
      }
    },
