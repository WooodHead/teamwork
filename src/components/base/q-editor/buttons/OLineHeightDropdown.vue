<template>
  <div>
    <q-btn-dropdown
      ...
      menu-self="top left"
      class="o-line-height-dropdown"
      content-class="o-menu o-line-height-dropdown-menu"
      dense
      flat
    >
      <q-list>
    ...
    <q-tooltip :delay="500">{{$t('editor.lineHeight')}}</q-tooltip>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      types: [
        { label: this.$t('editor.default'), value: '0' },
  ...
  methods: {
    ...
    itemClick (item) {
      this.commands.lineHeight({ lineHeight: item.value })
