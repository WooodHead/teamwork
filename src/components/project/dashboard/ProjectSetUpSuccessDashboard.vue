...
export default {
  name: 'ProjectSetUpSuccessDashboard',
  ...
  data () {
    ...
    return {
      initOption: {
        ...
        series: [
          {
            name: that.$t('dashboard.NewProjectNumber'),
            type: 'line',
            smooth: false,
            data: []
          }
  ...
  computed: {
    ...
    option () {
      ...
      for (let i = 0; i < 53; i++) { // 一年约53周
        ...
        // 上一周立项数
        weekNews.push(_.filter(data, (p) => {
          let beginDate = new Date(p.BeginDate.replace(/-/g, '/'))
          return +beginDate > +lastWeek && +beginDate <= +baseDate
        }).length)
        baseDate = lastWeek
      }
      that.initOption.xAxis.data = _.reverse(weeks)
      that.initOption.series[0].data = _.reverse(weekNews)
