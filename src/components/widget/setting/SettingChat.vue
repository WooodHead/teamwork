    ...
    <q-dialog v-model="showDialog">
      <q-card style="min-width:25%">
        <q-card-section class="q-gutter-md">
          ...
          <div>
            <q-checkbox
              v-model="onlyLeaderAndMember"
              label="只有负责人和成员可以收到聊天消息"
            />
          </div>
          <q-btn
            class="q-px-sm"
  ...
  methods: {
    ...
    init () {
      if (this.$route.name === 'settingsWidget') {
        this.type = 'resource'
        if (this.widget.setting.onlyLeaderAndMember) this.onlyLeaderAndMember = true
        ...
        this.$store.dispatch(`${this.category}/load${this.capitalize(this.category)}`, +this.objectID).then(res => {
          if (res.widgets && res.widgets.chat && res.widgets.chat.onlyLeaderAndMember) this.onlyLeaderAndMember = true
    ...
    updateWidget () {
      if (this.type) {
        if (this.type === 'resource') {
          this.widgetObj.setting.onlyLeaderAndMember = this.onlyLeaderAndMember
        } else {
          this.widgetObj.chat.onlyLeaderAndMember = this.onlyLeaderAndMember
        }
