    ...
    >
      <template v-slot:default-header="prop">
        <div class="row items-center">
          <div class="col" @click.stop="treedEvent(prop.node)">
            {{ prop.node.name }}
          </div>
          <div class="col-auto q-pl-sm" v-if="!showForm&&treed.id===prop.node.id">
            <q-icon
              v-ripple
              name="add"
              color="green-9"
...
export default {
  ...
  components: {
    'OrganizeForm': () => import('components/organize/OrganizeForm')
  },
  computed: {
